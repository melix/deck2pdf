...
First, download the TTF file for each font you want to use and copy it to +src/main/resources+. The files should be named using the hyphenated form shown in the CSS above. For example:

* YanoneKaffeesatz-Regular.ttf
* YanoneKaffeesatz-Bold.ttf

Next, open the +Browser.java+ source file and add the following declarations at the top of the constructor. These declarations load the fonts from the classpath:

[source,java]
----
Font.loadFont(getClass().getClassLoader().getResourceAsStream("YanoneKaffeesatz-Regular.ttf"), -1);
Font.loadFont(getClass().getClassLoader().getResourceAsStream("YanoneKaffeesatz-Bold.ttf"), -1);
----

Don't forget to add the following import to the +Browser.java+ source file:
...
Finally, build the deck2pdf distribution. You'll now be able to use the YanoneKaffeesatz font in your presentation when using JavaFX 2.2.
